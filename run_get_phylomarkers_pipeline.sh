...
# SMRtest_logs=( $(ls *.fasta) ) # <<< should be avoided
...
readarray -t SMRtest_logs < <(find . -maxdepth 1 -type f -name "*.fasta" -printf '%f\n') # -printf '%f\n' avoids ./ preceding files
...
then
    ...
    then
        ...
        msg "# This issue can be easily avoided by running the containerized version available from https://hub.docker.com/r/vinuesa/get_phylomarkers!" PROGR GREEN
        no_kde_outliers=0 
        no_kde_ok=$(cat all_gene_trees.tre | wc -l)
    else
