...
ok( eval{ `cd test_sequences/core_genome && ../../run_get_phylomarkers_pipeline.sh -R 1 -t DNA -n 10 | grep "wrote file gene_trees" && rm -rf get_phylomarkers_run_*` }, 'run_get_phylomarkers_pipeline.sh -R 1 -t DNA' ); 

# test 19 IQT run on proteins sequence ...
ok( eval{ `cd test_sequences/core_genome && ../../run_get_phylomarkers_pipeline.sh -R 1 -t PROT -k 1.2 -m 0.6  -n 10 | grep "wrote file" && rm -rf get_phylomarkers_run_*` }, 'run_get_phylomarkers_pipeline.sh -R 1 -t PROT -k 1.2 -m 0.6' ); 

# test 20 Thorough FastTree searching and molecular clock analysis on DNA sequences using 10 cores and increasing k stringency
...
# test 21 FastTree thorough searching on a protein dataset with moderate average bipartition support
ok( eval{  `cd test_sequences/core_genome && ../../run_get_phylomarkers_pipeline.sh  -R 1 -t PROT -A F -T high -m 0.6  -n 10 | grep "wrote file concat_nonRecomb_KdeFilt_protAlns_FTlgG"` }, 'run_get_phylomarkers_pipeline.sh -R 1 -t PROT -A F ...' );

# test 22  Run in population-genetics mode (generates a table with descritive statistics for DNA-polymorphisms) with K2P model
ok( eval{ `cd test_sequences/core_genome && ../../run_get_phylomarkers_pipeline.sh -R 2 -t DNA -S K2P  -n 10 | grep "wrote file polymorphism_descript_stats.tab"` }, 'run_get_phylomarkers_pipeline.sh -R 2 -t DNA -S K2P' );

# test 23 estimate a ML pan-genome tree from the pan-genome matrix, using 2 independent IQT runs and UFBoot
ok( eval{ `cd test_sequences/pan_genome && ../../estimate_pangenome_phylogenies.sh -f pangenome_matrix_t0.fasta -r 2 -S UFBoot  -n 10 | grep "done!"` }, 'estimate_pangenome_phylogenies.sh -r 2 -S UFBoot ...' );

# test 24 estimate a PARS  pan-genome tree with bootstrapping; 50 bootstrap replicates divided on 10 core (5 reps / core)
ok( eval{ `cd test_sequences/pan_genome && ../../estimate_pangenome_phylogenies.sh -c PARS -R 3 -i pangenome_matrix_t0.phylip -n 10 -b 5 -j 1 -t 1  -n 10 | grep "seq_key"` }, 'estimate_pangenome_phylogenies.sh -c PARS -R 3 ...' );

# test 24, run hcluster_pangenome_matrix.sh  
#ok( eval{ `cd test_sequences/pan_genome && ../../hcluster_pangenome_matrix.sh -i pangenome_matrix_t0.tab -T "Pan-genome tree" -a ward.D2 -d gower -O pdf -A 'NULL,45' -X 0.8 -s sil | grep 'hcluster_pangenome_matrix_script'` }, ' hcluster_pangenome_matrix.sh -i pangenome_matrix_t0.tab ...' );
