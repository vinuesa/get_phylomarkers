...
8. A **small sequence dataset is provided** with the distribution in the test_sequences/ directory for easy and fast testing (~16-60 seconds on a commodity GNU/Linux desktop machine with 4 cores; see [**GET_PHYLOMARKERS tutorial**](docs/GET_PHYLOMARKERS_manual.md#get_phylomarkers-tutorial), depending on the search thoroughness and/or nuber of models to be evaluated). 
...
[**GET_HOMOLOGUES**](https://github.com/eead-csic-compbio/get_homologues) package, as shown in the [**GET_PHYLOMARKERS tutorial**](docs/GET_PHYLOMARKERS_manual.md#get_phylomarkers-tutorial).
  ...
  NOTE: **both .faa and .fna files are required** to generate codon alignments from DNA fasta files. This
            means that two runs of *compare_clusters.pl* (from the **GET_HOMOLOGUES** package) are required,
	          one of them using the -n flag [**GET_PHYLOMARKERS tutorial**](docs/GET_PHYLOMARKERS_manual.md#get_phylomarkers-tutorial. 
...
*run_get_phylomarkers_pipeline.sh* uses a **hierarchical filtering scheme**, summarized in [Fig. 1](docs/GET_PHYLOMARKERS_manual.md#brief-presentation-and-graphical-overview-of-the-pipeline) and detailed below:
...
The alignments passing the two previous filters [Fig. 1](docs/GET_PHYLOMARKERS_manual.md#brief-presentation-and-graphical-overview-of-the-pipeline) are subjected to **maximum likelihood (ML) tree searches** with **FastTree** or **IQ-TREE** (default) to 
